...
func NewRemoteStorage(iprod GGOrderInfoProducer,
...
func (s *RemoteStorage) GetGGOrderInfoByIDs(ctx context.Context, IDs []uint64) ([]*models.GGOrderInfo, error) {
	log.Printf("RemoteStorage.GetGGOrderInfoByIDs: not implemented!")
	// log.Printf("remote_storage: GetGGOrderInfoByIDs called")
	// var ggorders []*models.GGOrderInfo
	// for _, id := range IDs {
	// 	ggorder, succ := s.cache.Get(context.Background(), strconv.FormatUint(id, 10))
	// 	if succ {
	// 		log.Printf("redis get: success %v", id)
	// 		ggorders = append(ggorders, ggorder)
	// 	} else {
	// 		log.Printf("redis get: record doesn't exist %v", id)
...
func (s *RemoteStorage) InsertGGOrderInfo(ctx context.Context, ggorderInfos []*models.GGOrderInfo) error {
	log.Printf("RemoteStorage.InsertGGOrderInfo: not implemented!")
	...
	// 	if err != nil {
	// 		log.Printf("redis set: success %+v", info)
